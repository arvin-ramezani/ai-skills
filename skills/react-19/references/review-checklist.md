...
**Required** for review mode and non-trivial PRs. **Optional** for typo-sized single-line edits (still apply JSX/Hooks write-time gates from `SKILL.md`). Prefer ESLint / `eslint-plugin-react-hooks` when available; do not claim pass without running project scripts.
